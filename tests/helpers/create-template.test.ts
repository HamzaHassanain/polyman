...
describe('create-template.ts', () => {
  ...
  describe('copyTemplate', () => {
      it('should copy files and directories recursively', () => {
          ...
          // verify file copy
          expect(fs.copyFileSync).toHaveBeenCalledWith(path.join(srcDir, 'file.txt'), path.join(destDir, 'file.txt'));
          
          // verify dir creation
          expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(destDir, 'subdir'), { recursive: true });
          
          // verify recursion (readdirSync called for subdir)
          expect(fs.readdirSync).toHaveBeenCalledWith(path.join(srcDir, 'subdir'), { withFileTypes: true });
      });
  });
