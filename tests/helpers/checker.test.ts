...
describe('checker.ts', () => {
  ...
  describe('ensureCheckerExists', () => {
    it('should not throw if checker is defined', () => {
      expect(() => checker.ensureCheckerExists({ name: 'test', source: 'test.cpp' })).not.toThrow();
    });

    it('should throw if checker is undefined', () => {
      vi.mocked(utils.throwError).mockImplementation(() => { throw new Error('Checker not defined'); }); // manual mock implementation needed since auto-mock does nothing
      ...
      expect(() => checker.ensureCheckerExists(undefined)).toThrow('No checker defined');
    });
  });
  ...
  describe('runChecker', () => {
    ...
    it('should pass if executor succeeds and expected verdict is OK', async () => {
        vi.mocked(executor.execute).mockResolvedValue({ stdout: '', stderr: '', exitCode: 0, success: true });
        
        await expect(checker.runChecker(mockExecCommand, mockInput, mockOutput, mockAnswer, 'OK')).resolves.not.toThrow();
    ...
    it('should throw if expected OK but executor reports error (WA)', async () => {
        ...
        vi.mocked(executor.execute).mockImplementation(async (cmd, options) => {
            if (options?.onError) {
                options.onError({ stdout: '', stderr: 'Wrong Answer', exitCode: 1, success: false });
            }
            return { stdout: '', stderr: 'Wrong Answer', exitCode: 1, success: false };
        });

        await expect(checker.runChecker(mockExecCommand, mockInput, mockOutput, mockAnswer, 'OK'))
