...
describe('utils.ts', () => {
    ...
    describe('filterTestsByRange', () => {
        ...
        it('should filter by range', () => {
             expect(utils.filterTestsByRange(testFiles, 2, 3)).toEqual(['test2.txt', 'test3.txt']);
        });
        it('should ignore non-test files during filtering', () => {
             expect(utils.filterTestsByRange(testFiles, 1, 3)).toEqual(['test1.txt', 'test2.txt', 'test3.txt']);
        });
    ...
    describe('getTestFiles', () => {
        it('should return sorted test files', () => {
             vi.mocked(fs.readdirSync).mockReturnValue(['test10.txt', 'test2.txt', 'other.txt'] as any);
    ...
    describe('Compilation', () => {
        describe('compileCPP', () => {
            ...
            it('should throw if file is not .cpp', async () => {
                 await expect(utils.compileCPP('main.c')).rejects.toThrow(/Expected .cpp file/);
            });
        });
    ...
    describe('Directories', () => {
        ...
        describe('removeDirectoryRecursively', () => {
             it('should remove directory if exists', () => {
                 ...
                 expect(fs.rmSync).toHaveBeenCalledWith(expect.stringContaining('del-dir'), { recursive: true, force: true });
             });
    ...
    describe('readFirstLine', () => {
         it('should return first line from stream', async () => {
              ...
              vi.mocked(fs.createReadStream).mockReturnValue({ on: mockOn, close: vi.fn() } as any);
         ...
         it('should handle empty file', async () => {
          ...
          it('should handle stream error', async () => {
             ...
             vi.mocked(fs.createReadStream).mockReturnValue({ on: mockOn, close: vi.fn() } as any);
             await expect(utils.readFirstLine('file.txt')).rejects.toThrow('Stream Error');
         });
    });
    ...
    describe('getCompiledCommandToRun', () => {
         ...
         it('should throw on unknown extension', () => {
              ...
              expect(() => utils.getCompiledCommandToRun(obj)).toThrow(/Unsupported source file extension/);
         });
    ...
    describe('Error Handling', () => {
        ...
        it('throwError should format error', () => {
             expect(() => utils.throwError('str error', 'ctx')).toThrow(/ctx: str error/);
        });
    });
});
