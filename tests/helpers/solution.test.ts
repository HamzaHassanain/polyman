...
describe('solution.ts', () => {
  ...
  describe('findMatchingSolutions', () => {
    const solutions: any[] = [{ name: 'main', source: 'main.cpp', type: 'main-correct' }, { name: 'wa', source: 'wa.cpp', type: 'wrong-answer' }];
    ...
    it('should throw if solution not found', () => {
      expect(() => solution.findMatchingSolutions(solutions, 'missing')).toThrow();
    });
  });
  ...
  describe('runSolutionOnSingleTest', () => {
      it('should execute solution successfully', async () => {
         ...
         const mockSolution = { name: 'main', source: 'main.cpp', type: 'main-correct' } as any;
         await expect(solution.runSolutionOnSingleTest(mockSolution, {} as any, 'testsets', 1)).resolves.not.toThrow();
      });
  });
});
