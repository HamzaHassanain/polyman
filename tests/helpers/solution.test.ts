...
describe('solution.ts', () => {
  ...
  describe('validateSolutionsExist', () => {
    it('should not throw if solutions exist', () => {
      const solutions: LocalSolution[] = [{ name: 'sol1', source: 'sol1.cpp', tag: 'MA' }];
      expect(() => solution.validateSolutionsExist(solutions)).not.toThrow();
    });

    it('should throw if solutions is undefined', () => {
      expect(() => solution.validateSolutionsExist(undefined)).toThrow('No solutions defined');
    });

     it('should throw if solutions is empty', () => {
      expect(() => solution.validateSolutionsExist([])).toThrow('No solutions defined');
    });
  });
  ...
  describe('findMatchingSolutions', () => {
    ...
    it('should return all solutions if name is all', () => {
      expect(solution.findMatchingSolutions(solutions, 'all')).toEqual(solutions);
    });

    it('should return matching solution', () => {
      expect(solution.findMatchingSolutions(solutions, 'main')).toEqual([solutions[0]]);
    });

    it('should throw if solution not found', () => {
      expect(() => solution.findMatchingSolutions(solutions, 'missing')).toThrow(/Solution named "missing" not found/);
    });
  });
  ...
  describe('compileSolution', () => {
    ...
    it('should throw on unsupported extension', async () => {
        ...
        await expect(solution.compileSolution('sol.txt')).rejects.toThrow('Unsupported solution file extension: .txt');
    });
  });
  ...
  describe('compileAllSolutions', () => {
      it('should compile all solutions', async () => {
          ...
          vi.spyOn(path, 'extname').mockImplementation((p) => p.endsWith('.cpp') ? '.cpp' : '.java');
  ...
  describe('runSolutionOnSingleTest', () => {
    ...
    it('should execute solution successfully', async () => {
      ...
      await solution.runSolutionOnSingleTest(mockSolution, mockConfig, 'testsets', 1);
     ...
     it('should throw if execution fails', async () => {
         vi.mocked(executor.executeWithRedirect).mockRejectedValue(new Error('Execution failed'));
         await expect(solution.runSolutionOnSingleTest(mockSolution, mockConfig, 'testsets', 1)).rejects.toThrow('Execution failed');
     });

    it('should handle runtime error (onError callback)', async () => {
        vi.mocked(executor.executeWithRedirect).mockImplementation(async (_cmd, options: any) => {
        ...
        await expect(solution.runSolutionOnSingleTest(mockSolution, mockConfig, 'testset1', 1))
            .rejects.toThrow('Runtime Error: Segfault');
        
        expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('output_test1.txt'), 'Runtime Error: Segfault');
    });
    ...
    it('should handle timeout (onTimeout callback)', async () => {
        ...
        expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('output_test1.txt'), expect.stringContaining('Time Limit Exceeded'));
    });
    ...
    it('should handle memory exceeded (onMemoryExceeded callback)', async () => {
        ...
        expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('output_test1.txt'), expect.stringContaining('Memory Limit Exceeded'));
    });
  });

  describe('runSolutionOnTestset', () => {
      const mockSolution: LocalSolution = { name: 'main', source: 'main.cpp', tag: 'MA' };
      ...
      it('should throw aggregate error if tests fail', async () => {
          ...
          vi.mocked(executor.executeWithRedirect).mockRejectedValue(new Error('Fail'));

          await expect(solution.runSolutionOnTestset(mockSolution, mockConfig, 'testset1'))
            .rejects.toThrow(/Solution .* failed on testset/);
      });
  });

  describe('runSolutionOnGroup', () => {
        const mockSolution: LocalSolution = { name: 'main', source: 'main.cpp', tag: 'MA', sourceType: 'cpp.g++17' };
        const mockConfig: ConfigFile = { timeLimit: 1000, memoryLimit: 256 } as any;
        ...
        it('should run tests belonging to group', async () => {
             ...
             await solution.runSolutionOnGroup(mockSolution, mockConfig, mockTestset, 'g1');

             expect(executor.executeWithRedirect).toHaveBeenCalledTimes(2);

             expect(executor.executeWithRedirect).toHaveBeenCalledWith(expect.anything(), expect.anything(), expect.stringContaining('test1.txt'), expect.anything());
             expect(executor.executeWithRedirect).toHaveBeenCalledWith(expect.anything(), expect.anything(), expect.stringContaining('test3.txt'), expect.anything());
        });
        ...
        it('should throw if no tests found for group', async () => {
             ...
             await expect(solution.runSolutionOnGroup(mockSolution, mockConfig, mockTestset, 'g1'))
  ...
  describe('runSolutionOnAllTestsets', () => {
        ...
        it('should run on all testsets', async () => {
             ...
             await solution.runSolutionOnAllTestsets(mockSolution, mockConfig, testsets);
             expect(executor.executeWithRedirect).toHaveBeenCalledTimes(2);
        });
        ...
        it('should aggregate errors from multiple testsets', async () => {
             ...
             await expect(solution.runSolutionOnAllTestsets(mockSolution, mockConfig, testsets))
  ...
  describe('ensureMainSolutionExists', () => {
      it('should pass if MA solution exists', () => {
          expect(() => solution.ensureMainSolutionExists([{ name: 'm', source: '', tag: 'MA', sourceType: 'cpp.g++17' }])).not.toThrow();
      });
      ...
      it('should throw if no MA solution', () => {
           expect(() => solution.ensureMainSolutionExists([{ name: 'w', source: '', tag: 'WA', sourceType: 'cpp.g++17' } as any])).toThrow(/No solution with tag "MA"/);
      });
  });

  describe('ensureSolutionExists', () => {
      it('should pass if solution exists', () => {
          expect(() => solution.ensureSolutionExists([{ name: 'm', source: '', tag: 'MA', sourceType: 'cpp.g++17' }], 'm')).not.toThrow();
      });
      it('should throw if solution missing', () => {
          expect(() => solution.ensureSolutionExists([{ name: 'm', source: '', tag: 'MA', sourceType: 'cpp.g++17' }], 'z')).toThrow(/No solution named "z"/);
      });
  });

  describe('getMainSolution', () => {
       it('should return MA solution', () => {
           const sol = { name: 'm', source: '', tag: 'MA', sourceType: 'cpp.g++17' } as LocalSolution;
  ...
  describe('testSolutionAgainstMainCorrect', () => {
      ...
      it('should run main then target then compare', async () => {
          ...
          vi.mocked(checker.runChecker).mockRejectedValue(new Error('Wrong Answer'));
      ...
      it('should fail if main solution fails', async () => {
          vi.mocked(executor.executeWithRedirect).mockRejectedValueOnce(new Error('Main fail'));
          await expect(solution.testSolutionAgainstMainCorrect('wa')).rejects.toThrow(/Failed to test solution "wa"/);
      });

     it('should fail if comparison process fails', async () => {

         vi.mocked(utils.getCompiledCommandToRun).mockImplementation(() => { throw new Error('Compilation missing'); });
         
         await expect(solution.testSolutionAgainstMainCorrect('wa')).rejects.toThrow(/Compilation missing/);
     });
  });

  describe('startTheComparisonProcess', () => {
       it('should validate expected verdicts', async () => {
           const mainSol: LocalSolution = { name: 'm', source: '', tag: 'MA', sourceType: 'cpp.g++17' };
           const targetSol: LocalSolution = { name: 'w', source: '', tag: 'WA', sourceType: 'cpp.g++17' } as any;
           ...
           await expect(solution.startTheComparisonProcess(checkerConfig, mainSol, targetSol, testsets)).resolves.not.toThrow();
       });
       ...
       it('should throw if verdict does not match expected', async () => {
           ...
           await expect(solution.startTheComparisonProcess({ name: 'chk', source: '' }, mainSol, targetSol, testsets))
  ...
  describe('Extended Branch Coverage', () => {
       describe('Verdict Validation via startTheComparisonProcess', () => {
           ...
           beforeEach(() => {
               ...
               vi.mocked(utils.readFirstLine).mockImplementation(async (p: string) => {
           ...
           it('should pass if solution tag is TL and it gets TLE', async () => {
               const mainSol: LocalSolution = { name: 'sol_main', source: '', tag: 'MA', sourceType: 'cpp.g++17' };
               const targetSol: LocalSolution = { name: 'sol_target', source: '', tag: 'TL', sourceType: 'cpp.g++17' } as any;
           ...
           it('should fail if solution tag is MA but gets TLE', async () => {
              const mainSol: LocalSolution = { name: 'sol_main', source: '', tag: 'MA', sourceType: 'cpp.g++17' };
              const targetSol: LocalSolution = { name: 'sol_target', source: '', tag: 'MA', sourceType: 'cpp.g++17' } as any;
              ...
              await expect(solution.startTheComparisonProcess(checkerConfig, mainSol, targetSol, testsets))
                  .rejects.toThrow(/marked as.*MA.*but got.*Time Limit Exceeded/);
           });

           it('should pass if solution tag is ML and it gets MLE', async () => {
              const mainSol: LocalSolution = { name: 'sol_main', source: '', tag: 'MA', sourceType: 'cpp.g++17' };
              const targetSol: LocalSolution = { name: 'sol_target', source: '', tag: 'ML', sourceType: 'cpp.g++17' } as any;
           ...
           it('should pass if solution tag is RE and it gets RTE', async () => {
              const mainSol: LocalSolution = { name: 'sol_main', source: '', tag: 'MA', sourceType: 'cpp.g++17' };
              const targetSol: LocalSolution = { name: 'sol_target', source: '', tag: 'RE', sourceType: 'cpp.g++17' } as any;
           ...
           it('should fail if tag is TL but did not get TLE', async () => {
               ...
               await expect(solution.startTheComparisonProcess(checkerConfig, mainSol, targetSol, testsets))
                   .rejects.toThrow(/marked as.*TL.*but did not get.*Time Limit Exceeded/);
           });

           it('should fail if tag is WA but did not get WA', async () => {
               const mainSol: LocalSolution = { name: 'sol_main', source: '', tag: 'MA', sourceType: 'cpp.g++17' };
               const targetSol: LocalSolution = { name: 'sol_target', source: '', tag: 'WA', sourceType: 'cpp.g++17' } as any;
       ...
       describe('Control Flow', () => {
           it('should stop running testset after first failure', async () => {
                ...
                vi.mocked(utils.getTestFiles).mockReturnValue(['t1.txt', 't2.txt', 't3.txt']);
                // Fail on first
                vi.mocked(executor.executeWithRedirect).mockRejectedValueOnce(new Error('Fail'));

                await expect(solution.runSolutionOnTestset(mockSolution, mockConfig, 'ts1')).rejects.toThrow();
           ...
           it('should stop running group after first failure', async () => {
                ...
                await expect(solution.runSolutionOnGroup(mockSolution, mockConfig, mockTestset, 'g1')).rejects.toThrow();
                expect(executor.executeWithRedirect).toHaveBeenCalledTimes(1);
           });
       });
       ...
       describe('Filesystem', () => {
           it('should ensure output directory is created', async () => {
                ...
                await solution.runSolutionOnSingleTest(mockSolution, mockConfig, 'ts1', 1);

                expect(fs.mkdirSync).toHaveBeenCalledWith(expect.stringContaining('main/ts1'), expect.anything());
           });
